  ...
  success: function(data) {
    ...
    for (var i = 0; i < data.length; i++){
      if (data[i].title == "Follow Request"){
        newHTML += "<div class='notification-box' id='notification-" + i.toString() + "' style='height:13vw;'><h3 class='notification-type'>" + data[i].title + "<img class='notification-close' onclick='CloseNotification(\"" + i.toString() + "\")'></h3><img class='notification-picture' src='" + data[i].profilePic + "'><p class='notification-name'>" + data[i].name + "</p><div class='follow-accept-button' onclick='AcceptRequest(\"" + data[i].name + "\",\"" + i.toString() + "\")'>Accept</div></div>";
      }
      else{
